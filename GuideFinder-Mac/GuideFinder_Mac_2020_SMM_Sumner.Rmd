...
Note: 
This file is adapted from the original GuideFinder R script for running on Sumner. 

Accessing files on Sumner: use scp to copy over files needed to run GuideFinder (GuideFinder script, genome fasta file, and refGenome)

Also make sure that you've downloaded BLAST (and GitHub, if needed) on Sumner.

Log in to Sumner: ssh [username]@login.sumner.jax.org

Starting an interactive job on Sumner: https://jacksonlaboratory.sharepoint.com/sites/ResearchIT/SitePages/Interactive-Jobs-with-SLURM.aspx
example: srun --nodes=1 --ntasks-per-node=1 --time=01:00:00 --pty bash -i

Loading R Studio on Sumner: https://jacksonlaboratory.sharepoint.com/sites/ResearchIT/SitePages/Running-Rstudio-on-Sumner.aspx
module load rstudio
rstudio-session-4.2.1
*then paste link into browser and log in with provided username and password

If running the code causes issues with locating files, try using an absolute file path instead of a relative file path.
...
install.packages("/projects/oh-lab/sierra/Guide-Finder/refGenome_1.7.7.tar.gz", repos = NULL, type = "source")
```
...
TestingStrings_test <- function(x) {
  str_detect(x,"AAAA")
}
...
If BLAST is not yet installed, follow directions here (Linux) after logging in to Sumner: https://www.ncbi.nlm.nih.gov/books/NBK52640/ 
...
```{r}
MakeDatabase<-'~/ncbi-blast-2.14.0+/bin/makeblastdb -in E_coli_CP017100.txt -dbtype nucl -out GenomeDatabase'
...
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("/projects/oh-lab/sierra/Guide-Finder/E_coli_CP017100.txt")
...
Sys.time()
# 
# FilteredGuides_test <-cbind(GuidesWithFeatures$GeneProduct,
#                       GuidesWithFeatures$Guide1,
#                       GuidesWithFeatures$GC_G1, 
#                       lapply(GuidesWithFeatures$Guide1,TestingStrings_test))
...
FilteredGuides3<-subset(FilteredGuides2, 
                        ...
                        select=ID:StartofGuideLocation)
#remove those with strings of As 
...
GuidesHits <- GuidesHits %>%
  ...
  mutate(StartOfGuideLocation = FilteredGuides6$StartofGuideLocation)
write.csv(GuidesHits, file="GuidesHitsBLAST.csv")
