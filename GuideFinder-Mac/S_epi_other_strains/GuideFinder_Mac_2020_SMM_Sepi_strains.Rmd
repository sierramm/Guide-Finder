...
Note: This file is adapted from the original GuideFinder R script for running with various S. epidermidis strains for guides with NGG, NAN, or NNN PAM sites.  

##
##1.Set Working Directory and Define Functions
The working directory should be where your FASTA file is stored & where all files will output to 

```{r setup}
#install.packages("knitr")

knitr::opts_knit$set(root.dir = '/Users/mellos/Documents/GitHub/Guide-Finder/GuideFinder-Mac')
setwd ('/Users/mellos/Documents/GitHub/Guide-Finder/GuideFinder-Mac')
```
...
# installing version 3.16 (July 2023)
...
had to download an archived version of refgenome: https://cran.r-project.org/src/contrib/Archive/refGenome/

```{r}
install.packages("doBy")
install.packages("~/Downloads/refGenome_1.7.7.tar.gz", repos = NULL, type = "source")
```
...
No edits need to be made to this section; this chunk just lists the functions that are used later in the script
```{r,tidy=TRUE}
...
# if searching for PAMless (NNN guides), locates any site.
# if searching for NAN guides, locates "NTN" sites.
# Otherwise, locates "CC" sites.
FindCC<-function (Sequences) {
  if (PAM_site == "NNN") {
    str_locate_all(Sequences,".")
  } else if (PAM_site == "NAN") {
    str_locate_all(Sequences,".T.")
  } else if (PAM_site == "NGG") {
    str_locate_all(Sequences,"CC")
  }
}

#function to find PAM site (promoter sequence only)
# if searching for PAMless (NNN guides), locates any site.
# if searching for NAN guides, locates "NAN" sites.
# Otherwise, locates "GG" sites.
FindGG<-function (Sequences) {
  if (PAM_site == "NNN") {
    str_locate_all(Sequences,".")
  } else if (PAM_site == "NAN") {
    str_locate_all(Sequences,".A.")
  } else if (PAM_site == "NGG") {
...
#extract NGG/PAM sequence
ExtractPAM <- function(x) {
...
GuideDistance<-function (x) {Df2<-as.matrix(dist(x$Location))
  ...
  Df6<-merge(Df5, x, by="Location") %>%
    dplyr::select(PAM_Guide1 = Var1, PAM_Guide2 = Var2, PAM_Dist = value, 
           Guide1 = Guide.x, Guide2 = Guide.y, Gene = GeneProduct.y, ID = ID.y)
  return(Df6)
  }
...
# commented out because this code is repeated in optional scrambles chunk
# not necessary here; also causes an issue in running
...
If BLAST is not yet installed, follow directions here (Mac): https://www.ncbi.nlm.nih.gov/books/NBK52640/ 
Make sure to modify commands to match most updated version of BLAST (2.14.0 as of July 2023).
If desired, modify .bash_profile as explained in configuration section so that BLAST can be called from any directory.
...
```{r}
MakeDatabase<-'~/ncbi-blast-2.14.0+/bin/makeblastdb -in 3B05_concatenatedfastafile.txt -dbtype nucl -out GenomeDatabase'

RunBlast<-'~/ncbi-blast-2.14.0+/bin/blastn -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'

ScrambleBlast<-'~/ncbi-blast-2.14.0+/bin/blastn -task blastn-short -query BLASTscrambles.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 11 -perc_identity .9 -out MatchScrambles.blast'

```


##3.Input FASTA File
Add the name of the fasta file that contains the whole nucleotide sequence for your genome (not the file with the individual coding sequences); if this was a draft genome, be sure that you used the preprocessing script to concatenate the several contigs into one.

Input a name of your choosing for your genome. This information is used when generating the parameters file and as a file prefix to the output files. 
...
GenomeName <- __"Strep_mutans"__
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("3B05_concatenatedfastafile.txt")
...
If you are searching for PAMless guides (NNN or NAN), you will get a large number of guides per gene. Unless interested in the promoter region specifically or looking at a specific gene that did not have any guides, consider setting promoter region to 0 to limit the output.
```{r}
PromoterRegion<-50
...
__Example__: Substitute the bolded information for the GenBank Accession # for your genome of interest.   
GenBankInfo<- readGenBank(GBAccession( __"AE014133.2"__ ))   
```{r,tidy=TRUE}
#retrieves annotation from GenBank, called by accession
GenBankInfo<- readGenBank(GBAccession("CP017100.1")) 
...
Input pre-processed input file (containing product/gene name, strand type, and start/end coordinates)
This loop takes the gene coordinates and modifies them to include the putative promoter region 
__Example__: Substitute the bolded information for the name of your file which contains the gene coordinates (output from custom or supplied pre-processing scripts). File should be kept in the working directory.  
...
CoordinatesDF<-fread("3B05_NewCoordinates.csv")
...
```{r,tidy=TRUE}
# E. coli homologs from high confidence essential genes identified in Spoto et al. https://journals.asm.org/doi/10.1128/mbio.02632-22#tabS1 
# c("NEB5A_00995", "NEB5A_00940", "NEB5A_19010", "NEB5A_16190", "NEB5A_20195", "NEB5A_04095", "NEB5A_17070")
# ftsB gene (NEB5A_14055): no NGG guides found with GuideFinder after filtering for off-target and distance from TSS
GenesToSelect<-c("NEB5A_14055")
ManualSelection<-FALSE
```



##7.Set Parameters for Guide Design
Change as desired. 
Set the following:
 ...
 *__PAM_site__: Set to "NGG", "NAN", or "NNN". For a completely PAMless search, choose "NNN".
 *__Find_Pairwise__: Set to TRUE or FALSE, if you want to find pairwise guides
 * __Run_Lowered_Thresholds__ : If looking for guides with lowered thresholds, set to TRUE. Otherwise set to FALSE.
 
 Note for PAMless guides: If running a PAMless search (NAN or NNN), you will likely get back a large number of guides. Pairwise selection will be very time-intensive, so  consider setting this to FALSE. Running with lowered thresholds is less time-intensive, but may not be needed. 

```{r,tidy=TRUE}
# parameters for guides
N <- 20 
GC_Min<-.35
DistanceAsPercentageofCDS<-.30
GuidesPerGene<-10
DistbtGuides<-100
Test_bad_seed <- TRUE 
Bad_seed <- "ACCCA"  
...
# parameters for which chunks to run
Find_Pairwise <- FALSE
Run_Lowered_Thresholds <- TRUE
...
If you are not choosing to run lower thresholds, no need to change these parameters or run this chunk.  

__Enter lowered thresholds below__
Change as desired: 
...
```{r,tidy=TRUE}
Re_RunBlast<-'~/ncbi-blast-2.14.0+/bin/blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_MatchGuides.blast'
#If selecting option to relax off-target searches 
Re_RunBlast_Lowered<-'~/ncbi-blast-2.14.0+/bin/blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_MatchGuides.blast'
```
...
__Create file with parameter data__
Generates a file to store user parameters for this run.
```{r,tidy=TRUE}

# Create data frame with parameters set above
Parameters_Df <- data.frame(Genome = GenomeName, PAM = PAM_site, Promoter_Length = PromoterRegion, Length_bp = N, 
                            GC_min = GC_Min, Dist_from_TSS = DistanceAsPercentageofCDS, Dist_bw_Pairs = DistbtGuides,
                            Test_Bad_Seed = Test_bad_seed, Bad_Seed_Seq = Bad_seed, Test_RE_Site = Test_RE,
                            RE_Seq = RE_site, Lowered_Thresholds = Run_Lowered_Thresholds, Lower_GC_Min = L_GC_Min, 
                            Lower_Dist_from_TSS = L_DistanceAsPercentageofCDS, Keep_A_String = KeepGuidesWithString, 
                            Keep_T_String = KeepGuidesWithString_T, Lower_Off_Target = LowerOffTarget)

# set lowered threshold parameters to NA if not running lowered thresholds
if (Run_Lowered_Thresholds == F) {
  Parameters_Df[,c("Lower_GC_Min", "Lower_Dist_from_TSS", "Keep_A_String", "Keep_T_String", "Lower_Off_Target")] <- NA
}

# create output file
write.csv(Parameters_Df, file = paste0(GenomeName, "_Parameters.csv"))

```


__For complete genomes, subsets genes if desired. IDs coordinates & alters them to include putative promoter sequence__  
This chunk of code is only performed on complete genomes, since the program identifies the coding sequence + promoter region from complete genomes using coordinates from the annotated Genbank file. By telling the program only to run the following if CompleteGenome == TRUE, this part is skipped if the user is utilizing a draft genome (specified by "CompleteGenome<-False"", above). For complete genomes, creates NewCoordinatesDF (for draft genomes, this data frame was made during preprocessing)
...
#ensure that no gene coordinates are past the end of the genome
NewCoordinatesDF <- NewCoordinatesDF %>%
  mutate(newend = case_when(end > length(InputGenome[[1]]) ~ length(InputGenome[[1]]),
                         end <= length(InputGenome[[1]]) ~ end)) %>%
  dplyr::select(geneproduct, strand, start, end=newend)
...
for (i in 1:nrow(CodingSequences)) {
  ...
  testing<-if (StrandType == "MINUS") 
    { replace(NewRow,3, str_c(rev(comp(s2c(NewRow$CDS), ambiguous = T)), collapse = ""))} 
...
__Finding Guide sites__  
Identifies guide sites in the gene body and the promoter. Guides can be targeted to the non-template strand *or* the template strand in the promoter region. However, guides can only be designed to target the non-template strand in the gene body. The first loop looks to identify potential guide sites in the gene body and promoter to create guides that target non-template strand. The second loop identifies guide sites in the promoter to create guides targeted to the template strand. 

Depending on the chosen PAM site (NGG, NAN, or NNN), this will locate all potential guide sites. 
```{r, eval=FALSE}
...
__Making guides given the PAM sites identified above__   
...
MakeGuidesList <- function(r, N){
  ...
  DistanceIntoCDS_AsPercentage<-(Start-PromoterRegion)/(LengthofCDS-PromoterRegion)
  ID<-ID
...
if (length(Prom_AllGuidesLocation) != 0) {
...
#create column with PAM sequence
CompleteGuidesList <- CompleteGuidesList %>%
  mutate(PAM = ExtractPAM(Guide3))

#remove NGG sequence from guide
...
write.csv(CompleteGuidesList, file=paste0(GenomeName, "_CompleteGuidesList.csv"))
...
Using the large guide with features data frame, begin to filter guides by GC content, distance into CDS, and if the guide has a homopolymer string. The output this is a data frame like GuidesWithFeatures but with an N/A in place of a value if that value does not meet the threshold we set; use NA to filter out guides
```{r, eval=FALSE}
...
# with dplyr
FilteredGuides <- GuidesWithFeatures %>%
  mutate(G1_AString = TestingStrings(Guide1),
         G2_AString = TestingStrings(Guide2),
         G3_AString = TestingStrings(Guide3),
         G1_TString = TestingStrings_T(Guide1),
         G2_TString = TestingStrings_T(Guide2),
         G3_TString = TestingStrings_T(Guide3),
         G1_Seed = TestingSeed(Guide1),
         G2_Seed = TestingSeed(Guide2),
         G3_Seed = TestingSeed(Guide3),
         G1_RE = TestingRE(Guide1),
         G2_RE = TestingRE(Guide2),
         G3_RE = TestingRE(Guide3)) %>%
  data.frame() %>%
  dplyr::select(ID, GeneProduct, Guide1, GC_G1, G1_AString, G1_TString, G1_Seed, G1_RE,
         Guide2, GC_G2, G2_AString, G2_TString, G2_Seed, G2_RE,
         Guide3, GC_G3=Gc_G3, G3_AString, G3_TString, G3_Seed, G3_RE,
         DistanceofGuide_AsPercentageofCDS = DistanceofFGuide_AsPercentageofCDS, StartofGuideLocation = StartofGuide)
...
CompleteGuidesList_Thresholds <- data.frame(lapply(FilteredGuides, as.character), 
                                            stringsAsFactors=FALSE)
CompleteGuidesList_Thresholds <- CompleteGuidesList_Thresholds %>%
  mutate(PAM = ExtractPAM(Guide3))
...
write.csv(CompleteGuidesList_Thresholds, 
          file=paste0(GenomeName, "_CompleteGuidesList_Thresholds.csv"), row.names = FALSE)

#Filtering Guides
# with dplyr
FilteredGuides1 <- FilteredGuides %>%
  filter(!G3_TString, !G3_AString) %>% # remove those with strings of As and Ts
  filter(!G3_Seed, !G3_RE) %>% # remove those with bad seed sequence or RE sequence
  filter(nchar(Guide1) >= N) %>% # remove those too short
  filter(DistanceofGuide_AsPercentageofCDS < DistanceAsPercentageofCDS)  %>% # remove those too far from start site
  filter(!grepl("N", Guide3)) # remove those with "N" - can't make guide if nt is unknown

#make into characters 
FilteredGuides1<-data.frame(lapply(FilteredGuides1, 
                                   as.character), stringsAsFactors = FALSE)
FilteredGuides1$GuideID<-1:nrow(FilteredGuides1) #add GUIDE ID to each guide
```
...
The output document (CompleteFilteredGuides) contains all potentially usable guides for a gene. The next step picks the desired # of guides/gene but the document output here may also be useful if a user wanted to manually select guides from a list of potential guides.
```{r, eval=FALSE,tidy=TRUE}
#shorten guide to 12 nt closest to PAM
#BLAST this shortened guide against entire genome
ToRemove<-Lengthof13(N) 
ShortenedGuides<-data.frame(cbind(FilteredGuides1$ID, 
                                  FilteredGuides1$GeneProduct,
                                  FilteredGuides1$Guide1,
                                  lapply(FilteredGuides1$Guide1, ShortenGuide)),
                            stringsAsFactors = F)
...
Hits <- Matchesdf %>%
  ...
  dplyr::select(hits, GuideID)

OneHit <-  Hits %>%
  filter(hits == 1)

GuidesWithoutOffTarget<-merge(OneHit, FilteredGuides1, by="GuideID")
GuidesWithoutOffTarget<-GuidesWithoutOffTarget[order(GuidesWithoutOffTarget$ID, decreasing = F),]

#Create file that indicates which guides have off target effects
GuidesHits <- merge(ShortenedGuides, Hits, by="GuideID")
GuidesHits <- GuidesHits %>%
  mutate(OffTarget = hits>1) %>%
  mutate(StartOfGuideLocation = FilteredGuides1$StartofGuideLocation)

write.csv(GuidesHits, file=paste0(GenomeName, "_GuidesHitsBLAST.csv"), row.names = F)
...
CompleteFilteredGuides<-data.frame(lapply(GuidesWithoutOffTarget, 
                                          as.character), stringsAsFactors=FALSE)
CompleteFilteredGuides <- CompleteFilteredGuides %>%
  mutate(PAM = ExtractPAM(Guide3))
...
write.csv(CompleteFilteredGuides, file= paste0(GenomeName, "_CompleteFilteredGuides.csv"))

```


__Final guide selection__  
Final guide selection; this is based on length and meeting GC threshold; if longest guide (guide3) at PAM site meets the GC threshold, it's chosen. If not, checks shorter guides at that PAM site. If none at that site, continues down list. Outputs top N guides per gene (selected by user).

Output: 

* Selects N number of guides for each gene where the max number of guides selected is designated by user (GuidesPerGene)  
    + For non-PAMless search (NGG): selects N number of guides closest to the start of each gene 
    + For a PAMless search (NAN or NNN): randomly selects N number of guides per gene from filtered list
    + TopHits_Guides
...
# Select useable guides
UseableGuides <- GuidesWithoutOffTarget %>%
  mutate(StartofGuideLocation = as.numeric(StartofGuideLocation),
         ID = as.numeric(ID)) %>%
  mutate(Guide = ifelse(CheckTheGC(GC_G3), Guide3, 
                          ifelse(CheckTheGC(GC_G2), Guide2, 
                                 ifelse(CheckTheGC(GC_G1), Guide1, "DoNotUse")))) %>%
  filter(Guide != "DoNotUse") %>%
  dplyr::select(Guide, GeneProduct, Location = StartofGuideLocation, ID)

# create PAM column and remove PAM sequence from guides
UseableGuides1 <- UseableGuides %>%
  mutate(PAM = ExtractPAM(Guide))

UseableGuides1 <- replace(UseableGuides1,1, 
                                RemoveNGG(UseableGuides$Guide)) 

# write output for all useable guides
write.csv(UseableGuides1, file = paste0(GenomeName, "_AllUseableGuides.csv"))

# Take number of guides per gene as specified in parameters
# output TopHits_GuideSelection
# if selecting PAMless guides, takes a random selection; otherwise takes guides closest to start of gene
if (PAM_site %in% c("NAN", "NNN")) {
  TopHits_GuideSelection <- UseableGuides %>%
  arrange(ID, Location) %>%
  group_by(ID) %>%
  slice_sample(n=GuidesPerGene)
} else {
  TopHits_GuideSelection <- UseableGuides %>%
  arrange(ID, Location) %>%
  group_by(ID) %>%
  slice_head(n=GuidesPerGene)
}
...
#add PAM column
TopHits_GuideSelection <-  TopHits_GuideSelection %>%
  mutate(PAM = ExtractPAM(Guide))
...
# create output file
write.csv(TopHits_GuideSelection, file = paste0(GenomeName, "_TopHits_Guides.csv"))

```


__Pairwise guide selection__  
Calculates the distance between the guides; guide pairs that are more distant than user set-threshold (Distbtguides) are output as acceptable guide pairs for double knockdown. NOTE: Optional to run if you don't need pairwise guides. Takes a long time to run.

Output: 
  
* Selects guide pairs suitable for double knockdown (in that the distance between the guides is greater than the minimum distance set by the user(DistbtnGuides)
    + Pairwise_Guides
    
```{r, eval=FALSE}
# Generate paired guides
# uses UseableGuides df, output Pairwise_GuideSelection df
Pairwise_GuideSelection<-NULL
AllIDs <- unique(UseableGuides$ID)

for (i in AllIDs) {
  GuidesDf <- UseableGuides %>% 
    filter(ID==i)
  
  if (dim(GuidesDf)[1] >=2) {
    SelectedGuides_Paired <- GuideDistance(GuidesDf)
...
# Create output file
write.csv(Pairwise_GuideSelection2, file = paste0(GenomeName, "_Pairwise_Guides.csv"))

```


__Genes without guides__  
Identifies genes that did not produce any usable Top Hits guides. This list is output in a .csv document called "GenesWithoutGuides" Thresholds can be lowered and this list of genes run through again. To identify and re-run genes that did not produce any PAIRED guides, then run the commented out code in the section below. 
```{r, eval=FALSE,tidy=TRUE}

# Identify genes that don't produce any Top Hits guides with the primary thresholds 
GenesWithPairs<-ifelse(Find_Pairwise, length(unique(Pairwise_GuideSelection2$ID)), NA)
GenesWithTopHits<-length(unique(TopHits_GuideSelection$ID))
GenesLost<-data.frame(length(unique(CodingSequences$ID)),
                      length(unique(CompleteGuidesList$ID)),
                                   GenesWithPairs,
                                   GenesWithTopHits,
                                   length(unique(CodingSequences$ID))-GenesWithTopHits)
names(GenesLost)<-c("Total_Genes", "Genes_With_Guide_Sites", "Genes_With_Guide_Pairs_Filtered", "Genes_With_Top_Hits_Filtered", "Genes_With_No_Hits")
write.csv(GenesLost, file=paste0(GenomeName, "_GenesLostInformation.csv"))
...
#output to file
write.csv(GenesWithoutGuide, file=paste0(GenomeName, "_GenesWithoutGuide.csv"))
...
__OPTIONAL: Make Scramble sequences__
Scramble sequences are 20 nt guide sequence that can act as a negative control. Scramble sequences are blasted against genome to make sure they do not target anywhere.
...
#write out file 
write.csv(file=paste0(GenomeName, "_Scrambles.csv"), Scrambles_NoHits)
...
If running PAMless search (NNN or NAN guides), you will likely get back a large number of genes, so this option may not be necessary.

Note: This chunk has not yet been updated to reflect changes to functions, filtering, and guide selection in previous chunks. 

```{r, eval=FALSE}
FeatureGuides_ID<-GuidesWithFeatures$ID

GuidesWithThresholds <- GuidesWithFeatures %>%
  mutate(G1_AString = TestingStrings(Guide1),
         G2_AString = TestingStrings(Guide2),
         G3_AString = TestingStrings(Guide3),
         G1_TString = TestingStrings_T(Guide1),
...
write.csv(Genes_ReFilter,file=paste0(GenomeName, "_GuidesPossibleForExcludedGenes.csv"))
...
# filter out those that don't meet distance threshold
GenesReFilter5 <- GenesReFilter4 %>%
  filter(DistanceAsPercentageofCDS < L_DistanceAsPercentageofCDS)
...
#output file 
write.csv(Re_FinalGuidesSelection, file=paste0(GenomeName, "_GuidesUsingLowerThresholds.csv"))
...
__Get Gene Info__
Outputs a file that includes information about the parameters used and the number of genes with guides. 
```{r}
if (Run_Lowered_Thresholds==F) {
  GenesGainedFromLoweringThresholds <- NA
}

# combine parameters information with amount of genes that produced guides
GeneInfoDF<-data.frame(GenesLost)
OverallGeneInfo<-data.frame(Parameters_Df, GeneInfoDF, 
                            GenesGainedFromLoweringThresholds) %>% 
  mutate(Total_Genes_With_Guides = Genes_With_Top_Hits_Filtered + GenesGainedFromLoweringThresholds)

# add info about number of guides
Useable_Guides_count <- length(UseableGuides1$ID)
Useable_Guides_lowered_thresholds <- length(Re_FinalGuidesSelection$ID)

OverallGeneInfo <- data.frame(OverallGeneInfo, Useable_Guides_count, Useable_Guides_lowered_thresholds) %>%
  mutate(Total_Useable_Guides = Useable_Guides_count + Useable_Guides_lowered_thresholds)

# create file with gene info 
write.csv(OverallGeneInfo, file=paste0(GenomeName, "_OverallGeneInfo.csv"))
