...
Note: This file is adapted from the original GuideFinder R script for designing guides for a specific S. epidermidis strain (NIHLM031: https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id=979214) 
Input genome file comes from running the pre-processing script. 

Note: currently configured to search for NAN PAMless guides.
...
setwd("/projects/oh-lab/sierra/Guide-Finder/GuideFinder-Mac/S_epi_6/")
```
...
```{r}
MakeDatabase<-'~/ncbi-blast-2.14.0+/bin/makeblastdb -in Sepi6concatenatedfastafile.txt -dbtype nucl -out GenomeDatabase'
...
##3.Input FASTA File
Add the name of the fasta file that contains the whole nucleotide sequence for your genome (not the file with the individual coding sequences); if this was a draft genome, be sure that you used the preprocessing script to concatenate the several contigs into one. Also set the name of your genome (used to create parameters file)
...
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("Sepi6concatenatedfastafile.txt")
GenomeName <- "Staph_epi_6_NNN"
```
...
CoordinatesDF<-fread("Sepi6_NewCoordinates.csv")
...
GenesToSelect<-c("NEB5A_00995", "NEB5A_00940", "NEB5A_19010", "NEB5A_16190", "NEB5A_20195", "NEB5A_04095", "NEB5A_17070")
...
If running PAMless search, you will likely get back a large number of genes, so this option may not be necessary. If not choosing to run lower thresholds, set Run_Lowered_Thresholds to FALSE and then run chunk. 
...
write.csv(Parameters_Df, file = "Sepi6_PAMless_Prom_Parameters.csv")
...
# ensure no gene coordiantes are past the end of the genome
...
write.csv(CompleteGuidesList, file="Sepi6_PAMless_Prom_CompleteGuidesList.csv")
...
write.csv(CompleteGuidesList_Thresholds, 
          file="Sepi6_PAMless_Prom_CompleteGuidesList_Thresholds.csv", row.names = FALSE)
...
write.csv(GuidesHits, file="Sepi6_PAMless_Prom_GuidesHitsBLAST.csv", row.names = F)
...
write.csv(CompleteFilteredGuides, file="Sepi6_PAMless_Prom_CompleteFilteredGuides.csv")
...
write.csv(UseableGuides, "Sepi6_PAMless_Prom_AllUseableGuides.csv")
...
# need to fix this: sample without replacement, if number of rows is less than GuidesPerGene then take them all
# temporary fix: sample with replacement, take only unique rows, results in less than # of GuidesPerGene for some genes
if (PAMless_guides == TRUE) {
  TopHits_GuideSelection <- UseableGuides %>%
  group_by(ID) %>%
  slice_sample(n=GuidesPerGene, replace = T) %>%
  unique() %>%
  arrange(ID, Location)
} else {
  TopHits_GuideSelection <- UseableGuides %>%
...
# create output file
write.csv(TopHits_GuideSelection, file = "Sepi6_PAMless_Prom_TopHits_Guides.csv")
...
# Create output file
write.csv(Pairwise_GuideSelection2, file = "Sepi6_PAMless_Prom_Pairwise_Guides.csv")
...
write.csv(GenesLost, file="Sepi6_PAMless_Prom_GenesLostInformation.csv")
...
#output to file
write.csv(GenesWithoutGuide, file="Sepi6_PAMless_Prom_GenesWithoutGuide.csv")
...
_Note_:  
Genes that did not produce guides is defined as genes that did not produce any gene pairs in the pairwise comparison. This includes genes that did not produce any useable guides at all _and_ genes that produced too few guides or guides too close together to be used as paired guides)
...
#output all guide possibilities for excluded genes into .csv file
#removing NGG prior to outputting 
...
write.csv(Genes_ReFilter,file="Sepi6_PAMless_Prom_GuidesPossibleForExcludedGenes.csv")
...
#output file 
write.csv(Re_FinalGuidesSelection, file="Sepi6_PAMless_Prom_GuidesUsingLowerThresholds.csv")
...
write.csv(OverallGeneInfo, file="Sepi6_PAMless_Prom_OverallGeneInfo.csv")
```
