...
```{r, eval=FALSE, echo=FALSE}
# S. epi 6 genome
fastafile<-read.fasta(file="S_epi_other_strains/NIHLM001_genomic.fna", as.string = TRUE) #read in fasta file
...
write.fasta(SingleSeqFasta,"SingleSequenceFastaFile",file="NIHLM001_concatenatedfastafile.txt") #write out to fasta file 
```
...
InputGenome<-readDNAStringSet("S_epi_other_strains/NIHLM001_concatenatedfastafile.txt", use.names = TRUE)
MakeDatabase<-'~/ncbi-blast-2.14.0+/bin/makeblastdb -in S_epi_other_strains/NIHLM001_concatenatedfastafile.txt -dbtype nucl -out GenomeDatabaseEpi'
#Input CDS file
CDS<- readDNAStringSet("S_epi_other_strains/NIHLM001_cds_from_genomic.fna", use.names = TRUE)
RunBlast<-'~/ncbi-blast-2.14.0+/bin/blastn -task megablast -query S_epi_other_strains/NIHLM001_cds_from_genomic.fna -db GenomeDatabaseEpi -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 1 -perc_identity 100 -out GenesMatch.blast'
...
write.csv(CorrectedCoordinates, file = "S_epi_other_strains/NIHLM001_NewCoordinates.csv", row.names = F )
```
