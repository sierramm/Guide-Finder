...
```{r}
MakeDatabase<-'makeblastdb -in E_coli_DH5alpha/E_coli_CP017100.txt -dbtype nucl -out GenomeDatabase'

RunBlast<-'blastn -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'

ScrambleBlast<-'blastn -task blastn-short -query BLASTscrambles.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 11 -perc_identity .9 -out MatchScrambles.blast'
...
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("E_coli_DH5alpha/E_coli_CP017100.txt")
 ...
 * __Run_Lowered_Thresholds__ : If looking for guides with lowered thresholds for genes that do not produce guides, set to TRUE. Otherwise set to FALSE.
...
```{r,tidy=TRUE}
Re_RunBlast<-'blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_MatchGuides.blast'
#If selecting option to relax off-target searches 
Re_RunBlast_Lowered<-'blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_MatchGuides.blast'
```
