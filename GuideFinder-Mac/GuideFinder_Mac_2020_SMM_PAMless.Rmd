...
Note: This file is adapted from the original GuideFinder R script to allow for designing guides that do not require a PAM site. 
...
# if searching for PAMless, locates any site. Otherwise, locates "CC" sites.
...
# if searching for PAMless, locates any site. Otherwise, locates "GG" sites.
...
If you are searching for PAMless guides, you will get a large number of guides per gene. Unless interested in the promoter region specifically or looking at a specific gene that did not have any guides, consider setting promoter region to 0 to limit the output.
```{r}
PromoterRegion<-0
 ...
 *__PAMless_guides__: Set to TRUE or FALSE, if you want to include guides made from any PAM site (FALSE will search for NGG guides only)
 ...
 * __Run_Lowered_Thresholds__ : If looking for guides with lowered thresholds, set to TRUE. Otherwise set to FALSE.
 ...
 Note for PAMless guides: If running a PAMless search, you will likely get back a large number of guides. Pairwise selection will be very time-intensive, so you consider setting this to FALSE. Running with lowered thresholds is less time-intensive, but may not be needed. 
...
Parameters_Df <- data.frame(Genome = GenomeName, PAMless = PAMless_guides, Promoter_Length = PromoterRegion, Length_bp = N, 
...
If PAMless_guides is TRUE (set by user in parameters), then this will locate every possible guide site. Otherwise, it will locate NGG/PAM sites. 
```{r, eval=FALSE}
...
* Selects N number of guides for each gene where the max number of guides selected is designated by user (GuidesPerGene)  
    + For non-PAMless search: selects N number of guides closest to the start of each gene 
    + For a PAMless search: randomly selects N number of guides per gene from filtered list
    + TopHits_Guides
...
If running PAMless search, you will likely get back a large number of genes, so this option may not be necessary.
